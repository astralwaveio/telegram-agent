...
def build_back_keyboard():
    return ReplyKeyboardMarkup([[KeyboardButton("返回主菜单")]], resize_keyboard=True, one_time_keyboard=False)
...
async def aichat_model_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if model == "返回主菜单":
        await update.effective_chat.send_message("已返回主菜单。", reply_markup=ReplyKeyboardMarkup([[]], resize_keyboard=True))
        return ConversationHandler.END
...
async def aichat_user_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_question.lower() in ["/cancel", "取消"]:
        await update.effective_chat.send_message("会话已取消。", reply_markup=ReplyKeyboardMarkup([[]], resize_keyboard=True))
        return ConversationHandler.END
